...
/* I don't want to deal with upper ascii bytes characters (which would be continuation bytes in UTF8
...
const char *escapeString(const char *strToEscape) {
	...
	for(;*ePtr;ePtr++) {
		assert(ptr < strEscapeBufferCapacity + strEscapeBuffer);
		if(escapeXlateTable[(int)*ePtr] != *ePtr) {
			ptr[0] = '\\';
			ptr[1] = escapeXlateTable[(int)*ePtr];
			ptr+=2;
		}
		else {
			ptr[0] = *ePtr;
...
void addInstance(unsigned long long id, unsigned long long classId, const char * typeCode, unsigned long long size) {
	...
	if(instancesFileBinary) {
		...
		assert(instancesSizeFileBinary);
...
int readPrimitiveArray(FILE *input) {
	...
	char typeCode = readByte(input);
	int bytesRead = identifierSize + 9;
	size_t elSize;
	...
	switch (typeCode) {
		...
		default : {
			fprintf(stderr, "unknown type for primitive array %d\n", typeCode);
			abort();
		}
	}
...
int readObjectArray(FILE *input) {
	long int id = readIdentifier(input);
	int stackTraceSerial = readInt(input);
	//StackTrace stackTrace = getStackTraceFromSerial(stackTraceSerial);
	int num = readInt(input);
	long int elementClassID = readIdentifier(input);
...
int readHeapDump(FILE *input, size_t dumpSize) {
	...
	while(positionInFile - dumpStartPosition < dumpSize) {
		...
		case HPROF_GC_OBJ_ARRAY_DUMP: {
		    int bytesRead = readObjectArray(input);
		    bytesLeft -= bytesRead;
		    break;
		}
		case HPROF_GC_PRIM_ARRAY_DUMP: {
		    int bytesRead = readPrimitiveArray(input);
		    bytesLeft -= bytesRead;
		    break;
		}
		default: {
...
int main(int argc, char **argv) {
	...
	if(strcmp(argv[3], "-") == 0) {
		...
		sprintf(fileNameBinary, "%s.size.binary", argv[3]);
